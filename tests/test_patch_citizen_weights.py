...
def test_legacy_v2_strip_aborts_when_its_landmark_moved(target):
    ...
    `_V2_BLOCK_RE` closes on a lookahead at the trailing bookkeeping comment.
    That comment is the only thing bounding a DOTALL `.*?`, so if it moves the
    match would run on to some later `});` and take live code with it. The step
    raises instead, and the patcher writes nothing at all -- no partial file, no
    backup implying one was made.
    """
    html = target.read_text(encoding="utf-8")
    ...
    target.write_text(
        html.replace(
            ...
            "const TOPIC_WEIGHTS = { housing: 1.0 };\n"
            "});\n"
            "\n// (this trailing comment is not the one the regex expects)\n\n"
            + marker,
            1,
        ),
        encoding="utf-8",
